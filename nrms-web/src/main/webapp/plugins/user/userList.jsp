...
<script type="text/javascript">
	...
	/* 修改，删除，重置密码 */
	function handleUser(m,id){
		debugger;
		switch (m){
			/* 修改 */
			case 1:
				window.location.href = "${basePath}/user/resetpwd?id="+id+"&oper=1";
				break;
			/* 删除 */
			case 2:
				deleteUser(id);
			...
			/* 重置密码 */
			case 3:
				window.location.href = "${basePath}/user/resetpwd?id="+id+"&oper=3";
				break;
			/* 新增用户 */
			case 4:
				window.location.href = "${basePath}/user/resetpwd?id="+id+"&oper=4";
				break;
			default:
				break;
		}
	...
	function addUser(){
		...
		$.ajax({
			type : 'POST',
			url : '${basePath}/user/toSaveOrEditUser'		
		})		
	}
													...
													<tbody>
														 <c:forEach var="user" items="${page.dataList }" varStatus="status">
															<tr>
																...
																<!-- 修改，删除，重置密码 -->
																<button onClick="handleUser(1,'${user.id}')" type="button" id="reedit" class="up-btn up-btn-primary up-btn-primary-red" data-toggle="modal" >修改</button>
																<button onClick="handleUser(2,'${user.id}')" type="button" id="redelete" class="up-btn up-btn-primary up-btn-primary-red" data-toggle="modal" >删除</button>
																<button onClick="handleUser(3,'${user.id}')" type="button" id="repwd" class="up-btn up-btn-primary up-btn-primary-red" data-toggle="modal" >重置密码</button>
